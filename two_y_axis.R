...
#分别绘制图
g1 <- ggplot(compa,aes(x=compa[,1],y=compa[,2])) + geom_line(colour = 'blue') + labs(y = 'disp') + theme_bw()
g2 <- ggplot(compa,aes(x=compa[,1],y=compa[,5])) + geom_line(colour = 'red') + labs(y = 'drat') + theme_bw() %+replace% 
    theme(panel.background = element_rect(fill = NA))
 
#使用 ggplot2.two_y_axis() 整合结果，并保存图片
ggsave('g1.jpg', g1)
ggsave('g2.jpg', g2)
ggsave('g12.jpg', ggplot2.two_y_axis(g1,g2))